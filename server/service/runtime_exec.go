...
func resolveManagedBinary(name string, preferredDirs []string, fallbackDirs []string) (string, error) {
	...
	// Fallback: search system PATH via exec.LookPath.
	// On Android, built-in runtimes are on PATH (set by the app shell),
	// and system tools like /system/bin/sh are also accessible this way.
	if path, err := exec.LookPath(name); err == nil {
		return path, nil
	}

	// Android fallback: if bash is not found, use sh (typically /system/bin/sh)
	if name == "bash" {

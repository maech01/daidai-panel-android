...
func Load(path string) (*Config, error) {
	...
	// Android: allow overriding data directories via environment variables
	// set by the native app shell.
	if envDataDir := os.Getenv("DAIDAI_DATA_DIR"); envDataDir != "" {
		cfg.Data.Dir = envDataDir
	}
	if envScriptsDir := os.Getenv("DAIDAI_SCRIPTS_DIR"); envScriptsDir != "" {
		cfg.Data.ScriptsDir = envScriptsDir
	}
	if envLogDir := os.Getenv("DAIDAI_LOG_DIR"); envLogDir != "" {
		cfg.Data.LogDir = envLogDir
	}

...
// 内存保护：长任务（备份/订阅同步）日志可达 MB 级，无上限累积会 OOM。
// 用环形缓冲：超过 MAX_LOG_LINES 就丢头部，保持可视区最新。
const MAX_LOG_LINES = 5000
// 单日志字节上限（双保险）：超 8MB 直接裁剪，避免拼接整个数组。
...
const renderedLogText = computed(() => {
  ...
  // 字节裁剪：单日志 8MB 封顶，超出截断到尾部
  // 注：JS string 是 UTF-16 code unit，1 char ≈ 1-3 bytes UTF-8。
  // 简单按 MAX_LOG_BYTES * 2 截可保证 ≤ 16MB UTF-8 字节，避免双重截断永远不收敛。
  if (text.length > MAX_LOG_BYTES * 2) {
    return text.slice(-MAX_LOG_BYTES * 2)
  }
  return text
})
